            ...
            <TransactionFormWrapper
                ...
                footer={
                    <div className="flex items-center justify-end gap-3 w-full">
                                    <button onClick={handleDelete} className="px-6 py-3 bg-white text-[#ff3b30] border-2 border-[#ff3b30] hover:bg-red-50 font-mono font-bold text-sm uppercase tracking-widest rounded-[3px] shadow-md shadow-red-100 transition-all active:scale-95 flex items-center justify-center gap-2 border-none">
                                    ...
                                    <button onClick={() => handleSave(false)} className="px-6 py-3 bg-white text-[#00adff] border-2 border-[#00adff] hover:bg-blue-50 font-mono font-bold text-sm uppercase tracking-widest rounded-[3px] transition-all active:scale-95 flex items-center justify-center gap-2 border-none">
                                        <Save size={14} /> SAVE 
                                    </button>
                                    <button onClick={() => handleSave(true)} className="px-6 py-3 bg-white text-[#2bb744] border-2 border-[#2bb744] hover:bg-green-50 font-mono font-bold text-sm uppercase tracking-widest rounded-[3px] shadow-md shadow-green-100 transition-all active:scale-95 flex items-center justify-center gap-2 border-none">
                                        <Check size={14} /> APPLY
                                    </button>
                    </div>
                }
            >
                <div className="space-y-4 font-['Tahoma',_sans-serif] text-slate-700">
                        ...
                        <div className="grid grid-cols-12 gap-x-4 gap-y-3.5">
                            ...
                            <div className="col-span-6 flex items-center gap-2">
                                ...
                                <div className="flex-1 flex gap-1 h-8 min-w-0">
                                    ...
                                    <input type="text" readOnly value={formData.custName} onClick={() => setShowCustModal(true)} className="flex-1 min-w-0 h-8 border border-slate-200 px-3 text-sm font-mono font-bold rounded outline-none bg-slate-50 text-gray-700 cursor-pointer transition-all appearance-none" placeholder=" "  style={{ backgroundImage: `url("data:image/svg+xml;charset=UTF-8,%3csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='none' stroke='%2364748b' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'%3e%3cpolyline points='6 9 12 15 18 9'%3e%3c/polyline%3e%3c/svg%3e")`, backgroundRepeat: 'no-repeat', backgroundPosition: 'right 0.5rem center', backgroundSize: '1em' }} />
                                </div>
                            </div>
                            ...
                            <div className="col-span-3 flex items-center gap-2">
                                ...
                                <div className="flex-1 flex gap-1 h-8 min-w-0">
                                    <input type="text" readOnly value={formData.payType} onClick={() => setShowPayTypeModal(true)} className="flex-1 min-w-0 h-8 border border-slate-200 px-3 text-sm font-mono font-bold text-gray-700 bg-slate-50 rounded outline-none cursor-pointer transition-all appearance-none"  style={{ backgroundImage: `url("data:image/svg+xml;charset=UTF-8,%3csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='none' stroke='%2364748b' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'%3e%3cpolyline points='6 9 12 15 18 9'%3e%3c/polyline%3e%3c/svg%3e")`, backgroundRepeat: 'no-repeat', backgroundPosition: 'right 0.5rem center', backgroundSize: '1em' }} />
                                </div>
                            </div>
                            ...
                            {rows.map((row, idx) => (
                                <div key={row.id} className="flex border-b border-slate-100 text-[11px] font-bold text-slate-700 hover:bg-blue-50/30 items-center transition-colors group">
                                    ...
                                    <div className="w-10 flex justify-center py-1">
                                        <button onClick={() => removeRow(row.id)} className="text-red-300 hover:text-red-500 transition-all p-1.5 hover:bg-red-50 rounded-[3px]">
                                            <Trash2 size={13} />
                                        </button>
                                    </div>
                                </div>
                    ...
                    <div className="flex flex-row justify-between items-end gap-x-12 mt-4">
                        ...
                        <div className="w-[360px] bg-white border border-slate-200 rounded-[3px] p-4 space-y-3 shadow-sm shrink-0">
                            {/* Total Row */}
                ...
                <SimpleModal isOpen={showProdModal} onClose={() => setShowProdModal(false)} title="Product Directory" maxWidth="max-w-[900px] min-h-[650px]">
                    <div className="space-y-4">
                        ...
                        <div className="border border-slate-200 rounded-[3px] overflow-hidden max-h-[500px] overflow-y-auto no-scrollbar">
                            <table className="w-full text-left">
                                <thead className="bg-[#f8fafc] sticky top-0 text-[11px] font-black text-gray-400 uppercase tracking-widest border-b border-gray-100 shadow-sm z-10">
                                    <tr>
                                        <th className=" px-5 py-3">Code</th>
                                ...
                                <table className="w-full text-left">
                                    ...
                                    <tbody className="divide-y divide-slate-100">
                                        {orders
                                            ...
                                            .length === 0 ? (
                                            <tr>
                                                <td colSpan="4" className="text-center py-16 text-gray-400 text-[11px] font-bold uppercase tracking-widest">Archive is currently empty</td>
                                            </tr>
                                        ) : orders
                                            .filter(o => !orderSearch || o.docNo.toLowerCase().includes(orderSearch.toLowerCase()) || o.date?.includes(orderSearch))
                                            .map((order, i) => (
