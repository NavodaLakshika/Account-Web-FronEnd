            ...
            <SimpleModal
                ...
                footer={
                    <div className="bg-slate-50 px-5 py-3.5 w-full flex justify-end gap-3 border-t border-gray-100 rounded-b-xl">
                        <button onClick={handleSave} disabled={loading} className={`px-5 h-8 bg-[#0078d4] text-white text-[12px] font-bold rounded-sm shadow-sm hover:bg-[#005a9e] transition-all active:scale-95 flex items-center justify-center gap-2 ${loading ? 'opacity-50' : ''}`}>
                            {loading ? <Loader2 size={13} className="animate-spin" /> : <Save size={13} />} Save
                        </button>
                        <button onClick={handleClear} disabled={loading} className="px-5 h-8 bg-white border border-gray-300 text-slate-600 text-[12px] font-bold rounded-sm hover:bg-slate-50 transition-all flex items-center justify-center gap-2">
                             <RotateCcw size={13} /> Clear
                        </button>
                        <button onClick={onClose} className="px-5 h-8 bg-white border border-gray-300 text-slate-600 text-[12px] font-bold rounded-sm hover:bg-slate-50 transition-all flex items-center justify-center gap-2">
                             <X size={13} /> Exit
                        </button>
                    </div>
                }
                    ...
                    <div className="bg-white p-5 border border-gray-100 rounded shadow-sm">
                        <div className="grid grid-cols-12 gap-x-8 gap-y-3.5">
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="flex-1 flex gap-1">
                                            <input 
                                                type="text" 
                                                readOnly 
                                                value={lookups.accounts.find(a => a.Account_Code === formData.debitAccount)?.Account_Name ? `${formData.debitAccount} - ${lookups.accounts.find(a => a.Account_Code === formData.debitAccount)?.Account_Name}` : ''} 
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="flex-1 flex gap-1">
                                            <input 
                                                type="text" 
                                                readOnly 
                                                value={lookups.accounts.find(a => a.Account_Code === formData.creditAccount)?.Account_Name ? `${formData.creditAccount} - ${lookups.accounts.find(a => a.Account_Code === formData.creditAccount)?.Account_Name}` : ''} 
                            ...
                            <div className="col-span-12 lg:col-span-5 space-y-3 bg-gray-50/20 p-4 border border-gray-100 rounded-sm">
                                ...
                                <div className="pt-4">
                                    <div className="p-3 bg-white border border-gray-100 rounded-sm shadow-sm space-y-1">
                                        <div className="text-[9px] font-bold text-slate-300 uppercase tracking-widest leading-none">SNAPSHOT</div>
                                        <div className="flex justify-between items-end">
                                            ...
                                            <div className="text-[8px] font-bold text-[#4cc3a5] bg-green-50 px-1.5 py-0.5 rounded uppercase">Verified</div>
                                        </div>
                                    </div>
                                </div>
                            </div>

                            {/* Full Width Memo at Bottom */}
                            <div className="col-span-12">
                                <div className="flex flex-col gap-1.5 pt-2">
                                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider pl-1">Memo / Remarks</label>
                                    <input 
                                        name="memo" 
                                        value={formData.memo} 
                                        onChange={handleInputChange} 
                    ...
                    <div className="relative w-full max-w-2xl bg-white shadow-2xl rounded-xl border border-gray-100 overflow-hidden flex flex-col max-h-[85vh]">
                        ...
                        <div className="overflow-y-auto p-2 font-['Plus_Jakarta_Sans']">
                            <table className="w-full text-sm text-left">
                                ...
                                <tbody>
                                    {(activeModal === 'bank' ? filteredData() : filteredData()).map((item, idx) => (
                                        <tr key={idx} className="hover:bg-blue-50 transition-colors group cursor-pointer" onClick={() => {
                                            ...
                                            <td className="p-3 border-b font-medium text-gray-700">{activeModal === 'bank' ? item.Bank_Code : item.Account_Code}</td>
                                            <td className="p-3 border-b font-medium uppercase text-blue-600">{activeModal === 'bank' ? item.Bank_Name : item.Account_Name}</td>
