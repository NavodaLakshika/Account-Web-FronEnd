            ...
            <TransactionFormWrapper
                ...
                footer={
                    <div className="bg-[#fcfcfc] px-6 py-4 w-full flex justify-between items-center border-t border-gray-200 rounded-b-[10px] shadow-[inset_0_1px_2px_rgba(0,0,0,0.02)]">
                        ...
                        <div className="flex gap-3">
                            <button onClick={handleSaveDraft} disabled={isApplying || items.length === 0} className="px-6 py-2 border-2 border-[#0285fd] text-[#0285fd] bg-white hover:bg-blue-50 font-semibold rounded-[3px] shadow-sm text-[13px] transition-all flex items-center justify-center gap-2 disabled:opacity-50">
                                {isApplying ? <Loader2 className="animate-spin" size={14} /> : <Save size={14} />} SAVE DRAFT
                            </button>
                            <button onClick={handleApply} disabled={isApplying || items.length === 0} className={`px-6 py-2 bg-[#0285fd] hover:bg-[#0073ff] text-white font-semibold rounded-[3px] shadow-sm text-[13px] transition-all flex items-center justify-center gap-2 ${(isApplying || items.length === 0) ? 'opacity-50 cursor-not-allowed' : ''}`}>
                        ...
                        <div className="grid grid-cols-12 gap-x-6 gap-y-3.5">
                            <div className="col-span-4">
                                ...
                                <div className="relative">
                                    <input type="text" readOnly value={formData.docNo} className="w-full h-10 border border-gray-300 rounded-[3px] px-3 text-[14px] bg-white outline-none focus:border-[#0285fd] focus:ring-1 focus:ring-[#0285fd] cursor-pointer pr-10 text-gray-700 truncate" />
                            ...
                            <div className="col-span-8">
                                ...
                                <div className="relative">
                                    <input type="text" readOnly value={lookups.customers.find(s => s.code === formData.customerId)?.name || ''} className="w-full h-10 border border-gray-300 rounded-[3px] px-3 text-[14px] bg-white outline-none focus:border-[#0285fd] focus:ring-1 focus:ring-[#0285fd] cursor-pointer pr-10 text-gray-700 truncate" onClick={() => setShowCustomerSearch(true)} />
                                    <button onClick={() => setShowCustomerSearch(true)} className="absolute right-1 top-1 bottom-1 w-8 flex items-center justify-center text-gray-500 hover:text-gray-800 bg-transparent border-none cursor-pointer"><Search size={16} /></button>
                                </div>
                            </div>
                            <div className="col-span-4">
                                <label className="block text-[13px] font-medium text-gray-700 mb-1.5">Payment Terms</label>
                                <div className="relative">
                                    <input type="text" readOnly value={formData.paymentTerms} className="w-full h-10 border border-gray-300 rounded-[3px] px-3 text-[14px] bg-white outline-none focus:border-[#0285fd] focus:ring-1 focus:ring-[#0285fd] cursor-pointer pr-10 text-gray-700 truncate" onClick={() => setShowTermsSearch(true)} />
                                    <button onClick={() => setShowTermsSearch(true)} className="absolute right-1 top-1 bottom-1 w-8 flex items-center justify-center text-gray-500 hover:text-gray-800 bg-transparent border-none cursor-pointer"><Search size={16} /></button>
                                </div>
                            </div>
