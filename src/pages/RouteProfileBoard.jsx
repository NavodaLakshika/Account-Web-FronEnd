...
const RouteProfileBoard = ({ isOpen, onClose }) => {
    ...
    const openRouteSearch = async () => {
        ...
        setLoading(true);
        try { const data = await routeService.searchRoutes(formData.Company, ''); setRouteList(data || []); setShowRouteSearch(true); } catch (err) { showErrorToast('Failed to load routes'); } finally { setLoading(false); }
    };

    const selectRoute = (route) => { setFormData(prev => ({ ...prev, Code: route.code, Route_Name: route.name })); setIsEditMode(true); setShowRouteSearch(false); };
                        ...
                        <div className="grid grid-cols-12 gap-x-6 gap-y-3.5">
                            <div className="col-span-6">
                                ...
                                <div className="relative">
                                    <input type="text" readOnly value={formData.Code || ''} onClick={openRouteSearch} className="w-full h-10 border border-gray-300 rounded-[3px] px-3 text-[14px] bg-white outline-none focus:border-[#0285fd] focus:ring-1 focus:ring-[#0285fd] pr-10 text-gray-700 font-mono cursor-pointer" />
                                    <button onClick={openRouteSearch} className="absolute right-1 top-1 bottom-1 w-8 flex items-center justify-center text-gray-500 hover:text-gray-800 bg-transparent border-none cursor-pointer"><Search size={16} /></button>
                                </div>
                            </div>
                            <div className="col-span-6">
                ...
                <div className="space-y-4 font-['Tahoma']">
                    <div className="flex items-center gap-4 bg-slate-50 p-4 border-b border-gray-100 mb-2">
                        ...
                        <div className="relative flex-1">
                            ...
                            <input type="text" placeholder="Find route..." className="w-full h-10 pl-10 pr-4 border border-gray-300 rounded-[3px] outline-none text-[13px] focus:border-[#0285fd] focus:ring-1 focus:ring-[#0285fd] shadow-sm bg-white" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} autoFocus />
                        </div>
                    </div>
                    <div className="border border-gray-200 rounded-[3px] overflow-hidden shadow-sm max-h-[400px] overflow-y-auto no-scrollbar">
                        <table className="w-full text-left">
                            ...
                            <tbody className="divide-y divide-gray-50">
                                {routeList.filter(r => (r.name || '').toLowerCase().includes(searchTerm.toLowerCase()) || (r.code || '').toLowerCase().includes(searchTerm.toLowerCase())).map((r, i) => (
