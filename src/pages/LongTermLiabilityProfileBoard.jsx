                        ...
                        <div className="grid grid-cols-12 gap-x-6 gap-y-3.5">
                            <div className="col-span-6">
                                ...
                                <div className="relative">
                                    <input type="text" readOnly value={(() => { const acc = lookups.accounts.find(a => (a.code || a.Code) === formData.LiabAccCode); return acc ? (acc.name || acc.Name || '').trim() : formData.LiabAccCode; })()} onClick={() => setShowAccountSearch(true)} className="w-full h-10 border border-gray-300 rounded-[3px] px-3 text-[14px] bg-white outline-none focus:border-[#0285fd] focus:ring-1 focus:ring-[#0285fd] pr-10 text-gray-700 cursor-pointer" />
                            ...
                            <div className="col-span-6">
                                ...
                                <div className="relative">
                                    <input type="text" readOnly value={(() => { const lender = lookups.lenders.find(l => (l.code || l.Code) === formData.LenderCode); return lender ? (lender.name || lender.Name || lender.supplier_Name || '').trim() : formData.LenderCode; })()} onClick={() => setShowLenderSearch(true)} className="w-full h-10 border border-gray-300 rounded-[3px] px-3 text-[14px] bg-white outline-none focus:border-[#0285fd] focus:ring-1 focus:ring-[#0285fd] pr-10 text-gray-700 cursor-pointer" />
                                    <button onClick={() => setShowLenderSearch(true)} className="absolute right-1 top-1 bottom-1 w-8 flex items-center justify-center text-gray-500 hover:text-gray-800 bg-transparent border-none cursor-pointer">
                                        <Search size={16} />
                                    </button>
                                </div>
                            </div>
