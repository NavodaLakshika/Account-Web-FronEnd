...
const CostCenterProfileBoard = ({ isOpen, onClose }) => {
    ...
    const handleSave = async () => {
        const newErrors = {};
        if (!formData.Name) newErrors.Name = 'Cost Center Name is required';
                ...
                <div className="space-y-3 overflow-y-auto no-scrollbar font-['Tahoma']">
                    <div className="bg-white p-4 border border-slate-200 rounded-[3px] space-y-4">
                        <div className="grid grid-cols-12 gap-x-6 gap-y-3.5">
                            ...
                            <div className="col-span-6">
                                <label className="block text-[13px] font-medium text-gray-700 mb-1.5">Cost Center Name <span className="text-red-500">*</span></label>
                                <input type="text" name="Name" value={formData.Name} onChange={handleInputChange} placeholder="Enter cost center name" maxLength={50} className={`w-full h-10 border ${errors.Name ? 'border-red-500' : 'border-gray-300'} rounded-[3px] px-3 text-[14px] bg-white outline-none focus:border-[#0285fd] focus:ring-1 focus:ring-[#0285fd] text-gray-700`} />
                                {errors.Name && <div className="text-red-500 text-[11px] mt-1">{errors.Name}</div>}
                            </div>
