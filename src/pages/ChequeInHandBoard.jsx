    ...
    const loadInitialData = async (compCode) => {
        try {
            ...
            const [lookupRes, docRes] = await Promise.all([
                bankingService.getChequeInHandLookups(activeComp),
                bankingService.generateDocNo('CIH', activeComp)
            ]);
                    ...
                    <div className="bg-slate-50/80 px-6 py-3 w-full flex justify-end gap-3 border-t border-slate-200 rounded-b-[5px]">
                        ...
                        <div className="flex gap-3">
                            <button onClick={handleSave} disabled={loading} className={`px-6 py-3 bg-[#2bb744] hover:bg-[#259b3a] text-white font-mono font-bold text-sm uppercase tracking-widest rounded-[5px] shadow-md shadow-blue-100 transition-all active:scale-95 flex items-center justify-center gap-2 border-none ${loading ? 'opacity-50' : ''}`}>
                                {loading ? <Loader2 size={14} className="animate-spin" /> : <CheckCircle2 size={16} />} COMMIT INVENTORY
                            </button>
                        </div>
                    </div>
                }
                ...
                <div className="space-y-4 font-['Tahoma']">
                        ...
                        <div className="grid grid-cols-12 gap-x-6 gap-y-3.5 relative z-10">
                            <div className="col-span-12 lg:col-span-8 flex items-center gap-2">
                                ...
                                <div className="flex-1 flex gap-1 h-8 min-w-0">
                                    <input type="text" readOnly value={header.bankName || header.bankCode} placeholder="Identify the bank for instrument custody..." className="flex-1 min-w-0 h-8 border border-slate-200 px-3 text-[12px] font-bold text-gray-700 bg-white rounded outline-none cursor-pointer transition-all focus:border-[#00D1FF] focus:ring-2 focus:ring-[#00D1FF]/20" onClick={() => { setActiveModal('bank'); setSearchTerm(''); }} />
                            ...
                            <div className="col-span-12 lg:col-span-4 flex items-center gap-2 lg:pl-4">
                                <label className="text-[11px] font-bold text-gray-500 uppercase w-24 shrink-0 text-right">Doc No</label>
                        ...
                        <table className="w-full text-left border-collapse flex-1 flex flex-col overflow-hidden">
                            <thead className="bg-slate-50 text-[10px] font-bold text-gray-500 uppercase tracking-widest sticky top-0 border-b border-slate-200 z-10 w-full flex">
                                <tr className="w-full flex">
                                    <th className="p-3 w-40 border-r border-slate-200">Cheque Date</th>
                                    <th className="p-3 flex-1 border-r border-slate-200">Memo / Reference</th>
                                    <th className="p-3 w-48 border-r border-slate-200">Cheque No</th>
                                    <th className="p-3 w-40 text-right pr-10 border-r border-slate-200">Amount (LKR)</th>
                            ...
                            <tbody className="divide-y divide-slate-100 overflow-y-auto block flex-1">
                                {items.length > 0 ? items.map((item, idx) => (
                                    <tr key={item.id} className="hover:bg-blue-50/50 transition-colors group flex w-full">
                                        <td className="p-3 w-40 border-r border-slate-100 font-mono text-slate-700 text-[12px] tabular-nums">{item.chequeDate}</td>
                                        <td className="p-3 flex-1 border-r border-slate-100 font-mono text-slate-700 text-[12px] italic">{item.memo || 'N/A'}</td>
                                        <td className="p-3 w-48 border-r border-slate-100 font-mono font-black text-[#0078d4] text-[13px] tracking-widest italic">{item.chequeNo}</td>
                                        <td className="p-3 w-40 text-right pr-10 border-r border-slate-100 font-mono font-black text-[#0285fd] text-[13px] tabular-nums italic group-hover:bg-white">{item.amount.toLocaleString(undefined, {minimumFractionDigits: 2})}</td>
                                    ...
                                    <tr className="flex w-full h-full">
                                        <td colSpan={5} className="p-10 text-center flex-1 flex flex-col items-center justify-center">
                                            <div className="flex flex-col items-center gap-4 opacity-20 grayscale">
                                                <HandCoins size={60} />
                                                <span className="text-[12px] font-bold uppercase tracking-[0.2em] text-slate-500">Inventory Empty</span>
                                            </div>
                                        </td>
                                    </tr>
                                )}
                    ...
                    <div className="bg-white p-4 border border-slate-200 rounded-[5px] space-y-3.5 relative">
                         ...
                         <div className="grid grid-cols-12 gap-x-6 gap-y-3.5 items-end">
                            <div className="col-span-2 flex flex-col gap-1.5">
                                ...
                                <div className="flex-1 flex gap-1 h-8 min-w-0">
                                    <input type="text" readOnly value={entry.chequeDate} className="flex-1 min-w-0 h-8 border border-slate-200 rounded px-3 text-[12px] font-bold outline-none bg-slate-50 text-gray-700 transition-all focus:border-[#00D1FF] focus:ring-2 focus:ring-[#00D1FF]/20 cursor-pointer" onClick={() => setShowDatePicker(true)} />
                            ...
                            <div className="col-span-4 flex flex-col gap-1.5">
                                <label className="text-[10px] font-bold uppercase text-gray-500 ml-1">Memo / Detail</label>
                                <input type="text" value={entry.memo} onChange={e => setEntry({...entry, memo: e.target.value})} placeholder="Specify context..." className="h-8 w-full border border-slate-200 px-3 text-[12px] font-mono rounded outline-none focus:border-[#00D1FF] focus:ring-2 focus:ring-[#00D1FF]/20 italic" />
                            </div>
                            <div className="col-span-2 flex flex-col gap-1.5">
                                <label className="text-[10px] font-bold uppercase text-gray-500 ml-1">Cheque No</label>
                    ...
                    <div className="bg-slate-50 p-4 rounded-[5px] border border-slate-200 flex justify-between items-center">
                        <div className="flex items-center gap-4">
                            <div className="p-2 bg-blue-100 rounded text-[#0285fd]">
                                <ShieldCheck size={20} />
                            </div>
                            <div className="space-y-0.5">
                                <span className="text-[10px] font-bold uppercase text-gray-500 tracking-widest leading-none">Security Protocol</span>
                                <h4 className="text-slate-700 text-[12px] font-bold">Physical Custody Verification Active</h4>
                            </div>
                        </div>
                        <div className="flex items-baseline gap-4 h-full pr-4">
                             <div className="flex flex-col items-end gap-0.5">
                                <span className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Aggregate Value</span>
                                <div className="flex items-baseline gap-2">
                                    <span className="text-[9px] font-black text-slate-400 italic">Rs.</span>
                                    <span className="text-[18px] font-mono font-black text-[#0285fd] tabular-nums italic">
