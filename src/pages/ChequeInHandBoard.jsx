                ...
                <div className="space-y-4 font-['Tahoma']">
                    ...
                    <div className="bg-white p-4 border border-slate-200 rounded-[3px]">
                        <div className="grid grid-cols-12 gap-x-6 gap-y-3.5">
                            <div className="col-span-8">
                                <label className="block text-[13px] font-medium text-gray-700 mb-1.5">Target Bank</label>
                        ...
                        <table className="w-full text-left border-collapse flex-1 flex flex-col overflow-hidden">
                            <thead className="bg-slate-50 text-[10px] font-black text-gray-400 uppercase tracking-widest leading-10 sticky top-0 border-b border-slate-200 z-10 w-full flex">
                                <tr className="w-full flex">
                                    <th className="px-4 w-40 border-r border-slate-200">Cheque Date</th>
                                    <th className="px-4 flex-1 border-r border-slate-200">Memo / Reference</th>
                                    <th className="px-4 w-48 border-r border-slate-200">Cheque No</th>
                                    <th className="px-4 w-40 text-right border-r border-slate-200">Amount (LKR)</th>
                            ...
                            <tbody className="divide-y divide-slate-100 overflow-y-auto block flex-1">
                                {items.length > 0 ? items.map((item, idx) => (
                                    <tr key={item.id} className="hover:bg-blue-50/50 transition-colors group flex w-full">
                                        <td className="px-4 w-40 border-r border-slate-100 font-mono text-slate-700 text-[12px] tabular-nums">{item.chequeDate}</td>
                                        <td className="px-4 flex-1 border-r border-slate-100 font-mono text-slate-700 text-[12px] italic">{item.memo || 'N/A'}</td>
                                        <td className="px-4 w-48 border-r border-slate-100 font-mono font-black text-gray-800 text-[13px] tracking-widest">{item.chequeNo}</td>
                                        <td className="px-4 w-40 text-right border-r border-slate-100 font-mono font-black text-[#0285fd] text-[13px] tabular-nums group-hover:bg-white">{item.amount.toLocaleString(undefined, {minimumFractionDigits: 2})}</td>
                    ...
                    <div className="bg-white p-4 border border-slate-200 rounded-[3px]">
                         ...
                         <div className="grid grid-cols-12 gap-x-6 gap-y-3.5 items-end">
                            <div className="col-span-2">
                                ...
                                <div className="relative">
                                    <input type="text" value={entry.chequeDate} className="w-full h-10 border border-gray-300 rounded-[3px] px-3 text-[14px] bg-white outline-none focus:border-[#0285fd] focus:ring-1 focus:ring-[#0285fd] pr-10 text-gray-700 truncate" name="chequeDate" onChange={(e) => setEntry(prev => ({ ...prev, chequeDate: e.target.value }))} />
                            ...
                            <div className="col-span-4">
                                <label className="block text-[13px] font-medium text-gray-700 mb-1.5">Memo / Detail</label>
                                <input type="text" value={entry.memo} onChange={e => setEntry({...entry, memo: e.target.value})} placeholder="Specify context..."
                    ...
                    <div className="bg-slate-50 p-3 rounded-[3px] border border-gray-200 flex justify-between items-center">
                        <div className="flex items-center gap-3">
                            <span className="text-[11px] font-bold text-gray-600 uppercase tracking-widest">Cheques Registered:</span>
