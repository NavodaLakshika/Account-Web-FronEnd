...
const DepartmentProfileBoard = ({ isOpen, onClose }) => {
    const initialState = { Code: '', Dept_Name: '', Loca_Id: '', Loca_Name: '', Company: '', CurrentUser: 'SYSTEM' };
    ...
    const [locaSearchTerm, setLocaSearchTerm] = useState('');
    ...
    const handleSave = async () => {
        if (!formData.Loca_Id || !formData.Dept_Name) { showErrorToast('Location and Department Name are required'); return; }
        setLoading(true);
        try {
            const data = await departmentService.save(formData);
            if (data.message === 'inserted') { showSuccessToast('Department created'); setFormData(prev => ({ ...prev, Code: data.code })); setIsEditMode(true); }
            else { showSuccessToast('Department updated'); }
    ...
    const confirmDelete = async () => {
        setLoading(true);
        try { await departmentService.delete(formData.Code, formData.Loca_Id, formData.Company); showSuccessToast('Department deleted'); handleClear(); setShowDeleteConfirm(false); } catch (err) { showErrorToast(err.message || err); } finally { setLoading(false); }
    };

    const openLocaSearch = async () => {
        if (!formData.Company) { showErrorToast('Company not identified'); return; }
        setLoading(true);
        try { const data = await departmentService.getAllLocations(formData.Company); setLocationsList(data || []); setShowLocaSearch(true); } catch (err) { showErrorToast('Failed to load locations'); } finally { setLoading(false); }
    };

    const selectLocation = (loc) => { setFormData(prev => ({ ...prev, Loca_Id: loc.code, Loca_Name: loc.name })); setShowLocaSearch(false); };

    const openDeptSearch = async () => {
        if (!formData.Loca_Id) { showErrorToast('Select a location first'); return; }
        setLoading(true);
        try { const data = await departmentService.searchDepartments(formData.Loca_Id, formData.Company, ''); setDeptList(data || []); setShowDeptSearch(true); } catch (err) { showErrorToast('Failed to load departments'); } finally { setLoading(false); }
    };

    const selectDept = (dept) => { setFormData(prev => ({ ...prev, Code: dept.code, Dept_Name: dept.name })); setIsEditMode(true); setShowDeptSearch(false); };
    ...
    return (
        <>
                ...
                footer={
                    <div className="bg-[#fcfcfc] px-6 py-4 w-full flex justify-between items-center border-t border-gray-200 rounded-b-[10px] shadow-[inset_0_1px_2px_rgba(0,0,0,0.02)]">
                        <div className="flex gap-3">
                            {isEditMode && <button onClick={handleDelete} className="px-6 py-2 border-2 border-red-500 text-red-600 bg-white hover:bg-red-50 font-semibold rounded-[3px] shadow-sm text-[13px] transition-all flex items-center justify-center gap-2"><Trash2 size={14} /> DELETE</button>}
                            <button type="button" onClick={handleClear} className="px-6 py-2 border border-gray-300 text-gray-700 bg-white hover:bg-gray-50 font-semibold rounded-[3px] shadow-sm text-[13px] transition-all flex items-center justify-center gap-2"><RotateCcw size={14} /> CLEAR</button>
                        </div>
                        <div className="flex gap-3">
                            <button onClick={handleSave} disabled={loading} className="px-6 py-2 bg-[#0285fd] hover:bg-[#0073ff] text-white font-semibold rounded-[3px] shadow-sm text-[13px] transition-all flex items-center justify-center gap-2 disabled:opacity-50"><Save size={14} /> SAVE</button>
                        </div>
                    </div>
                }
            >
                        ...
                        <div className="grid grid-cols-12 gap-x-6 gap-y-3.5">
                            <div className="col-span-12">
                                ...
                                <div className="relative">
                                    <input type="text" readOnly value={formData.Loca_Name || formData.Loca_Id} onClick={openLocaSearch} placeholder="Select location..." className="w-full h-10 border border-gray-300 rounded-[3px] px-3 text-[14px] bg-white outline-none focus:border-[#0285fd] focus:ring-1 focus:ring-[#0285fd] pr-10 text-gray-700 cursor-pointer" />
                                    <button onClick={openLocaSearch} className="absolute right-1 top-1 bottom-1 w-8 flex items-center justify-center text-gray-500 hover:text-gray-800 bg-transparent border-none cursor-pointer"><Search size={16} /></button>
                                </div>
                            </div>
                            <div className="col-span-6">
                                <label className="block text-[13px] font-medium text-gray-700 mb-1.5">Department ID</label>
                                <div className="relative">
                                    <input type="text" readOnly value={formData.Code || ''} onClick={openDeptSearch} className="w-full h-10 border border-gray-300 rounded-[3px] px-3 text-[14px] bg-white outline-none focus:border-[#0285fd] focus:ring-1 focus:ring-[#0285fd] pr-10 text-gray-700 font-mono cursor-pointer" />
                                    <button onClick={openDeptSearch} className="absolute right-1 top-1 bottom-1 w-8 flex items-center justify-center text-gray-500 hover:text-gray-800 bg-transparent border-none cursor-pointer"><Search size={16} /></button>
                                </div>
                            </div>
                            <div className="col-span-6">
            ...
            <SimpleModal isOpen={showLocaSearch} onClose={() => setShowLocaSearch(false)} title="Location Search" maxWidth="max-w-[700px]">
                <div className="space-y-4 font-['Tahoma']">
                    <div className="flex items-center gap-4 bg-slate-50 p-4 border-b border-gray-100 mb-2">
                        ...
                        <div className="relative flex-1">
                            ...
                            <input type="text" placeholder="Find location..." className="w-full h-10 pl-10 pr-4 border border-gray-300 rounded-[3px] outline-none text-[13px] focus:border-[#0285fd] focus:ring-1 focus:ring-[#0285fd] shadow-sm bg-white" value={locaSearchTerm} onChange={(e) => setLocaSearchTerm(e.target.value)} autoFocus />
                        </div>
                    </div>
                    <div className="border border-gray-200 rounded-[3px] overflow-hidden shadow-sm max-h-[400px] overflow-y-auto no-scrollbar">
                        <table className="w-full text-left">
                            <thead className="bg-[#f8fafc] sticky top-0 text-[11px] font-black text-gray-400 uppercase tracking-widest border-b border-gray-100 shadow-sm z-10">
                                <tr><th className=" px-5 py-3">CODE</th><th className=" px-5 py-3">LOCATION NAME</th><th className="text-right px-5 py-3">Action</th></tr>
                            </thead>
                            <tbody className="divide-y divide-gray-50">
                                {locationsList.filter(l => (l.name || '').toLowerCase().includes(locaSearchTerm.toLowerCase()) || (l.code || '').toLowerCase().includes(locaSearchTerm.toLowerCase())).map((loc, i) => (
                                    <tr key={i} className="group hover:bg-blue-50/50  transition-all cursor-pointer group border-b border-gray-50" onClick={() => selectLocation(loc)}>
                                        <td className="font-mono text-[12px] font-bold text-blue-600 px-5 py-3">{loc.code}</td>
                                        <td className="text-[12px] font-bold text-slate-700 uppercase group-hover:text-blue-600 transition-colors px-5 py-3">{loc.name}</td>
                                ...
                                {locationsList.length === 0 && <tr><td colSpan="3" className="text-center py-16 text-gray-400 text-[11px] font-bold uppercase tracking-widest">No locations found.</td></tr>}
                            </tbody>
                        </table>
                    </div>
                </div>
