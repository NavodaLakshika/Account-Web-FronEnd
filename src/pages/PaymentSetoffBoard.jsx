...
const PaymentSetoffBoard = ({ isOpen, onClose }) => {
    ...
    const handleApplySetoff = async () => {
        if (!formData.supplierId || selectedSetoffs.length === 0) {
            toast.error("Please select a supplier and allocate set-offs.");
            return;
        }
        ...
        try {
            ...
            await paymentSetoffService.apply({ ...formData, lines: selectedSetoffs });
             toast.success('Payment setoff applied successfully!');
            handleClear();
            onClose();
        } catch (error) {
                ...
                <div className="fixed inset-0 z-[1100] flex items-center justify-center p-4">
                    ...
                    <div className="relative w-full max-w-2xl bg-white shadow-2xl rounded-xl border border-gray-100 overflow-hidden flex flex-col max-h-[85vh] font-['Plus_Jakarta_Sans']">
                        <div className="p-4 border-b border-gray-200 flex justify-between items-center bg-gray-50">
                            ...
                            <button onClick={() => setActiveSupplierModal(false)} className="w-8 h-8 flex items-center justify-center hover:bg-red-50 text-slate-400 hover:text-red-500 transition-colors rounded-full transition-all"><X size={28} /></button>
                        </div>
