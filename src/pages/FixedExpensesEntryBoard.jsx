...
const FixedExpensesEntryBoard = ({ isOpen, onClose }) => {
    const initialState = {
        AccCode: '', ExpenseAccount: '', Vendor: '', DebitDate: new Date().toISOString().split('T')[0],
    ...
    const fetchLookups = async () => {
        try { const data = await fixedExpensesService.getLookups(); setLookups(data.accounts || []); setPayTypes(data.payTypes || []); } catch (error) { console.error('Lookup error:', error); }
    };

    const fetchList = async (company) => {
        try { const data = await fixedExpensesService.getList(company); setExpenseList(data || []); } catch (error) { console.error('List error:', error); }
    };
    ...
    const handleAccountSelect = (acc) => { setFormData(prev => ({ ...prev, AccCode: acc.code, ExpenseAccount: acc.name })); setShowAccSearch(false); };
    ...
    const handleSave = async () => {
        ...
        if (!formData.Vendor) { showErrorToast('Vendor name is required.'); return; }
        if (!formData.Amount || isNaN(formData.Amount)) { showErrorToast('Please enter a valid debit amount.'); return; }
    ...
    return (
        <>
                ...
                <div className="space-y-3 overflow-y-auto no-scrollbar font-['Tahoma']">
                        ...
                        <div className="grid grid-cols-12 gap-x-6 gap-y-3.5">
                            <div className="col-span-6">
                                ...
                                <div className="relative">
                                    <input type="text" readOnly value={formData.ExpenseAccount} onClick={() => setShowAccSearch(true)} className="w-full h-10 border border-gray-300 rounded-[3px] px-3 text-[14px] bg-white outline-none focus:border-[#0285fd] focus:ring-1 focus:ring-[#0285fd] text-gray-700 cursor-pointer appearance-none"  style={{ backgroundImage: `url("data:image/svg+xml;charset=UTF-8,%3csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='none' stroke='%2364748b' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'%3e%3cpolyline points='6 9 12 15 18 9'%3e%3c/polyline%3e%3c/svg%3e")`, backgroundRepeat: 'no-repeat', backgroundPosition: 'right 0.5rem center', backgroundSize: '1em' }} />
                                    {formData.AccCode && (
                                        <span className="absolute right-10 top-1/2 -translate-y-1/2 text-[10px] font-bold text-red-500 bg-red-50 border border-red-100 px-1.5 py-0.5 rounded">{formData.AccCode}</span>
                                    )}
                                </div>
                            </div>
                            <div className="col-span-6">
                            ...
                            <div className="col-span-6">
                                <label className="block text-[13px] font-medium text-gray-700 mb-1.5">Debit Date</label>
                                <div className="relative">
                                    <input type="text" readOnly value={formData.DebitDate} onClick={() => setShowCalendar(true)} className="w-full h-10 border border-gray-300 rounded-[3px] px-3 text-[14px] bg-white outline-none focus:border-[#0285fd] focus:ring-1 focus:ring-[#0285fd] pr-10 text-gray-700 cursor-pointer" />
                            ...
                            <div className="col-span-12">
                                <label className="block text-[13px] font-medium text-gray-700 mb-1.5">Debit Amount</label>
                    ...
                    <div className="bg-white border border-slate-200 rounded-[3px] overflow-hidden">
                        <div className="bg-slate-50 px-4 py-2.5 border-b border-slate-200 flex justify-between items-center">
                            <span className="text-[11px] font-bold text-gray-500 uppercase tracking-widest">Recent Fixed Expense Entries</span>
                            <span className="text-[10px] font-bold text-[#0285fd] bg-blue-50 px-2 py-0.5 border border-green-100 rounded">{expenseList.length} Entries Found</span>
                        </div>
                        <div className="max-h-[250px] overflow-y-auto no-scrollbar">
                            <table className="w-full text-left">
                                <thead className="bg-slate-50 sticky top-0 text-[10px] font-black text-gray-400 uppercase tracking-widest border-b border-gray-200">
                                    <tr><th className="px-4 py-2">Expense Source</th><th className="px-4 py-2">Vendor</th><th className="px-4 py-2 text-center">Date</th><th className="px-4 py-2 text-right">Amount</th><th className="text-right px-5 py-3">Action</th></tr>
                                </thead>
                                    ...
                                    {expenseList.map((item, idx) => (
                                        <tr key={idx} className="border-b border-slate-100 hover:bg-blue-50/50 transition-all group">
                                            <td className="px-4 py-2">
                                                <div className="text-[10px] font-bold text-red-500 font-mono mb-0.5">{item.accCode}</div>
                                                <div className="text-[11px] font-bold text-gray-700 uppercase leading-tight group-hover:text-red-500">{item.expenseAccount}</div>
                                            </td>
                                            ...
                                            <td className="px-4 py-2 text-[12px] font-black text-red-500 text-right">{parseFloat(item.amount).toLocaleString(undefined, { minimumFractionDigits: 2 })}</td>
                                        </tr>
                                    ))}
                                    {expenseList.length === 0 && (
                                        <tr><td colSpan="4" className="px-4 py-12 text-center text-gray-400 text-[11px] font-bold uppercase tracking-widest italic">No fixed expense records found for this company.</td></tr>
                                    )}
                                </tbody>
                            </table>
                        </div>
            ...
            <SimpleModal isOpen={showAccSearch} onClose={() => setShowAccSearch(false)} title="Expense Accounts Lookup" maxWidth="max-w-[700px]">
            ...
            <CalendarModal isOpen={showCalendar} onClose={() => setShowCalendar(false)} onDateSelect={(date) => { setFormData(prev => ({ ...prev, DebitDate: date })); setShowCalendar(false); }} initialDate={formData.DebitDate} />
        </>
    );
};

export default FixedExpensesEntryBoard;
