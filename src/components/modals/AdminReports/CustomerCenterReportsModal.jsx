...
import { X, ChevronRight, Users, Clock, FileText, BarChart3, Receipt, Search } from 'lucide-react';
...
const menuGroups = [
    {
        ...
        items: [
            { icon: Clock, label: 'Debtor Aging Analyst', desc: 'View debtor aging analysis', action: 'aging' },
            { icon: FileText, label: 'Debtors Statement', desc: 'View debtor statements', action: 'statement' },
            { icon: BarChart3, label: 'Debtors Balance Summary', desc: 'View debtor balance summary', action: 'balance' },
            { icon: Receipt, label: 'Invoice Details', desc: 'View customer invoice details', action: 'invoice' },
        ]
    }
];

const CustomerCenterReportsModal = ({ isOpen, onClose }) => {
        ...
        <div className="fixed inset-0 z-[300] flex items-center justify-center p-4">
            ...
            <div className="relative w-full max-w-2xl bg-white rounded-2xl shadow-2xl overflow-hidden border border-white/10 animate-in zoom-in-95 duration-300">
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-white">
                    <div className="flex items-center gap-3">
                        <div className="w-9 h-9 rounded-xl bg-[#4f83ff]/10 flex items-center justify-center">
                            <Users size={16} className="text-[#4f83ff]" />
                        </div>
                        <div>
                            <h2 className="text-[15px] font-black uppercase tracking-[0.25em] text-slate-900 leading-tight">
                                Customer Center Reports
                            </h2>
                            <p className="text-[10px] text-slate-400 font-medium tracking-wider">Debtor Reports &amp; Analytics</p>
                        </div>
                    </div>
                ...
                <div className="px-5 py-2.5 bg-slate-50/80 border-t border-slate-100 flex items-center justify-between">
                    ...
                    <span className="text-[9px] text-slate-300">Customer Reports</span>
                </div>
            </div>
        </div>
    );
...
export default CustomerCenterReportsModal;
