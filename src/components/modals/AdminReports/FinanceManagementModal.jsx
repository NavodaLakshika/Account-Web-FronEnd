...
import { X, ChevronRight, PieChart, BookOpen, Scale, FileText, BarChart3, Search } from 'lucide-react';
...
const menuGroups = [
    {
        ...
        items: [
            { icon: PieChart, label: 'Profit & Loss Account', desc: 'View profit and loss statements', action: 'pnl' },
            { icon: BookOpen, label: 'General Ledger', desc: 'View general ledger reports', action: 'ledger' },
            { icon: Scale, label: 'Trial Balance', desc: 'View trial balance reports', action: 'trialBalance' },
            { icon: FileText, label: 'Balance Sheet', desc: 'View balance sheet reports', action: 'balanceSheet' },
        ]
    }
];
...
const FinanceManagementModal = ({ isOpen, onClose }) => {
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-white">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-[10px] text-slate-400 font-medium tracking-wider">Financial Statements &amp; Reports</p>
                        </div>
                    </div>
 ...
 <div className="w-9 h-9 rounded-sm bg-white flex items-center justify-center shadow-sm group-hover:border-[#4f83ff]/20 group-hover:bg-[#4f83ff]/5 transition-all shrink-0">
                ...
                <div className="px-5 py-2.5 bg-slate-50/80 border-t border-slate-100 flex items-center justify-between">
                    ...
                    <span className="text-[9px] text-slate-300">Finance Reports</span>
                </div>
            </div>
        </div>
    );
