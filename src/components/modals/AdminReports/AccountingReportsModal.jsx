...
import { X, ChevronRight, FileText, BarChart3, BookOpen, ClipboardList, Search } from 'lucide-react';
...
const menuGroups = [
    {
        ...
        items: [
            { icon: FileText, label: 'Accounting Details', desc: 'View detailed accounting reports', action: 'details' },
            { icon: BarChart3, label: 'Accounting Report', desc: 'View summary accounting reports', action: 'report' },
            { icon: BookOpen, label: 'Journal Entry Reports', desc: 'View journal entry reports', action: 'journal' },
            { icon: ClipboardList, label: 'Double Entry Details Report', desc: 'View double entry details', action: 'doubleEntry' },
        ]
    }
];

const AccountingReportsModal = ({ isOpen, onClose }) => {
            ...
            <div className="relative w-full max-w-2xl bg-white rounded-2xl shadow-2xl overflow-hidden border border-white/10 animate-in zoom-in-95 duration-300">
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-white">
                    <div className="flex items-center gap-3">
                        <div className="w-9 h-9 rounded-xl bg-[#4f83ff]/10 flex items-center justify-center">
                            <BarChart3 size={16} className="text-[#4f83ff]" />
                        </div>
                        <div>
                            <h2 className="text-[15px] font-black uppercase tracking-[0.25em] text-slate-900 leading-tight">
                                Accounting Reports
                            </h2>
                            <p className="text-[10px] text-slate-400 font-medium tracking-wider">Journal &amp; Ledger Reports</p>
                        </div>
                    </div>
                ...
                <div className="px-5 py-3 bg-slate-50/80 border-b border-slate-100">
                    <div className="relative">
                        ...
                        <input
                            type="text"
                            placeholder="Search accounting reports..."
                            value={searchQuery}
                ...
                <div className="px-5 py-2.5 bg-slate-50/80 border-t border-slate-100 flex items-center justify-between">
                    ...
                    <span className="text-[9px] text-slate-300">Accounting Reports</span>
                </div>
            </div>
        </div>
    );
...
export default AccountingReportsModal;
