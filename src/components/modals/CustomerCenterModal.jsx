...
import { X, ChevronRight, ShoppingCart, FileText, CreditCard, Receipt, Banknote, Users, Search } from 'lucide-react';
...
const menuGroups = [
    {
        ...
        items: [
            { icon: ShoppingCart, label: 'Create Invoices', desc: 'Generate new sales orders and invoices', action: 'salesOrder' },
            { icon: FileText, label: 'Customer Invoices', desc: 'View and manage customer invoices', action: 'customerInvoice' },
            { icon: CreditCard, label: 'Received Payment', desc: 'Record payments received from customers', action: 'receivePayment' },
            { icon: Receipt, label: 'Customer Receipt', desc: 'Issue sales receipts to customers', action: 'salesReceipt' },
            { icon: Banknote, label: 'Customer Advanced Receive', desc: 'Manage advance payments from customers', action: 'customerAdvance' },
        ]
    }
];
...
const CustomerCenterModal = ({ isOpen, onClose, onOpenSalesOrder, onOpenCustomerInvoice, onOpenSalesReceipt, onOpenReceivePayment, onOpenCustomerAdvance }) => {
    ...
    const handleAction = (action) => {
        switch (action) {
            case 'salesOrder': onOpenSalesOrder(); break;
            case 'customerInvoice': onOpenCustomerInvoice(); break;
            case 'receivePayment': onOpenReceivePayment(); break;
            case 'salesReceipt': onOpenSalesReceipt(); break;
            case 'customerAdvance': onOpenCustomerAdvance(); break;
        }
        onClose();
    };
        ...
        <div className="fixed inset-0 z-[300] flex items-center justify-center p-4">
            ...
            <div className="relative w-full max-w-2xl bg-white rounded-2xl shadow-2xl overflow-hidden border border-white/10 animate-in zoom-in-95 duration-300">
                ...
                <div className="px-5 py-2.5 bg-slate-50/80 border-t border-slate-100 flex items-center justify-between">
                    ...
                    <span className="text-[9px] text-slate-300">Sales Records</span>
                </div>
            </div>
        </div>
    );
