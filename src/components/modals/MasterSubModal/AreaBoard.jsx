...
import { Search, RotateCcw, Save, Trash2, Loader2, X, MapPin } from 'lucide-react';
...
const AreaBoard = ({ isOpen, onClose }) => {
    ...
    const handleSave = async () => {
        if (!formData.Route_Code || !formData.Area_Name) {
    ...
    const openRouteSearch = async () => {
        ...
        try {
            ...
            showErrorToast('Failed to load routes');
        } finally {
            setLoading(false);
        }
    };
    ...
    const openAreaSearch = async () => {
        if (!formData.Route_Code) {
            showErrorToast('Select a route first');
            return;
        }
        setLoading(true);
        try {
    ...
    const footer = (
        <div className="w-full flex justify-end gap-3 border-t border-gray-100 rounded-b-xl">
            <button
                onClick={handleSave}
                disabled={loading}
    ...
    return (
        <>
            <SimpleModal isOpen={isOpen} onClose={onClose} title="Area Selection" maxWidth="max-w-[500px]" footer={footer}>
                <div className="py-2 select-none font-['Tahoma'] space-y-4 text-[12.5px] mt-1">
                    <div className="space-y-3">
                        {/* Route ID Row */}
                        <div className="flex items-center gap-6">
                            <label className="w-32 font-bold text-gray-700">Route ID</label>
                        ...
                        {/* Route Name Row */}
                        <div className="flex items-center gap-6">
                            <label className="w-32 font-bold text-gray-700">Route Name</label>
                            <input
                                type="text"
                        ...
                        {/* Area ID Row */}
                        <div className="flex items-center gap-6">
                            <label className="w-32 font-bold text-gray-700">Area ID</label>
                            <div className="flex-1 flex gap-3">
                                <input
                                    type="text"
                                    name="Code"
                        ...
                        {/* Area Name Row */}
                        <div className="flex items-center gap-6">
                            <label className="w-32 font-bold text-gray-700">Area Name</label>
                            <input
                                type="text"
                                name="Area_Name"
                ...
                <AdminConfigPanel entityType="AREA" entityCode={formData.Code} company={formData.Company} />
            </SimpleModal>
            ...
            <ConfirmModal
                ...
                title="Delete Area"
                message={`Are you sure you want to delete area ${formData.Area_Name || formData.Code}? This action cannot be undone.`}
                variant="danger"
                loading={loading}
                        ...
                        {list.filter(x => (x.name || '').toLowerCase().includes(query.toLowerCase()) || (x.code || '').toLowerCase().includes(query.toLowerCase())).map(x => (
                            ...
                            >
                                <div className="flex items-center gap-2 flex-1">
                                    <span className="w-24 text-center font-mono text-[11px] font-bold text-[#0078d4]">{x.code}</span>
                                    <span className="flex-1 px-3 font-mono font-medium text-gray-700 uppercase">{x.name}</span>
                                </div>
