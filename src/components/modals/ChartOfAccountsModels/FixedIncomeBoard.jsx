...
const FixedIncomeBoard = ({ isOpen, onClose }) => {
    ...
    const handleAccountSelect = (code, name) => {
        setFormData(prev => ({ ...prev, IncomeAccCode: code, IncomeAccName: name.trim() }));
    ...
    const handleSave = () => {
        if (!formData.IncomeAccCode || !formData.Buyer || !formData.Amount) {
    ...
    return (
        <>
            ...
            <div className="select-none font-['Tahoma'] space-y-4">
                    ...
                    <div className="bg-white p-4 border border-gray-200 rounded-[5px] space-y-3 shadow-sm border-l-4 border-l-[#0285fd]">
                        <div className="flex items-center gap-4">
                            ...
                            <div className="flex-1 flex gap-2">
                                <input 
                                    type="text" 
                                    value={formData.IncomeAccName || formData.IncomeAccCode} 
                                    readOnly 
                    ...
                    <div className="border border-gray-300 rounded-[5px] overflow-hidden flex flex-col min-h-[300px] bg-white shadow-sm">
                        <div className="flex bg-gray-50 border-b border-gray-300">
                            ...
                            <div className="flex-1 px-4 py-2 text-[11px] font-bold text-gray-600 border-r border-gray-200 uppercase tracking-wider text-center">Income Account</div>
                        ...
                        <div className="flex-1 overflow-y-auto bg-white custom-scrollbar">
                            {records.length === 0 ? (
                                <div className="p-12 text-center text-gray-400 italic text-sm">No recorded income entries found.</div>
                            ) : (
                                records.map((rec, idx) => (
                                    <div key={idx} className="flex border-b border-gray-100 hover:bg-slate-50 transition-colors group">
                                        ...
                                        <div className="flex-1 px-4 py-2 text-xs font-bold text-gray-700 uppercase">{rec.Buyer}</div>
                                        <div className="flex-1 px-4 py-2 text-xs font-medium text-gray-600 uppercase">{rec.IncomeAccName}</div>
                                        <div className="flex-1 px-4 py-2 text-xs font-bold text-gray-500 text-center">{rec.CreditDate}</div>
            ...
            <CalendarModal 
                ...
                currentDate={formData.CreditDate}
            />
        </>
    );
};
