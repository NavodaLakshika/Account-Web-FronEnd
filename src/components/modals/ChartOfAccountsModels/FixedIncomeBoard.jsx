        ...
        <>
            ...
            >
                <div className="select-none font-['Tahoma'] space-y-4 p-2">
                    {/* Main Form */}
                    <div className="bg-white p-6 border border-gray-200 rounded-[5px] space-y-5 shadow-sm border-l-4 border-l-[#0078d4]">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-5">
                            {/* Account Selection */}
                            <div className="space-y-2">
                                <label className="text-[11px] font-bold text-gray-700 uppercase tracking-wider">Income Account</label>
                                <div className="flex gap-2">
                                    <div className="relative flex-1">
                                        <input 
                                            ...
                                            readOnly 
                                            className="w-full h-9 border border-gray-300 px-3 text-[12.5px] bg-gray-50 rounded-[5px] outline-none font-bold text-blue-600 shadow-sm"
                                        />
                                        {formData.AccCode && (
                            ...
                            <div className="space-y-2">
                                <label className="text-[11px] font-bold text-gray-700 uppercase tracking-wider flex items-center gap-2">
                                    <User size={12} className="text-gray-400" /> Buyer Name
                                </label>
                                <input 
                                    name="Buyer"
                            ...
                            <div className="space-y-2 md:col-span-2">
                                <label className="text-[11px] font-bold text-gray-700 uppercase tracking-wider">Credit Amount</label>
                                <div className="relative">
                                    <input 
                                        ...
                                        type="number"
                                        placeholder="0.00"
                                        className="w-full h-12 border border-gray-300 px-4 text-[20px] focus:border-blue-500 outline-none rounded-[5px] font-black text-right text-slate-700 shadow-sm pr-12 bg-[#f8fafc]"
                                    />
                    ...
                    <div className="border border-gray-200 rounded-[5px] overflow-hidden shadow-sm bg-white">
                        <div className="bg-gray-50 px-4 py-3 border-b border-gray-200 flex justify-between items-center">
                            ...
                            <span className="text-[10px] font-bold text-gray-400 bg-white px-2 py-0.5 border rounded-full">{incomeList.length} Entries Found</span>
                        </div>
                        <div className="max-h-[250px] overflow-y-auto custom-scrollbar">
                            <table className="w-full text-left border-collapse">
                                <thead className="sticky top-0 bg-white z-10">
                                    <tr className="bg-slate-100 border-b border-gray-200 shadow-sm">
                                        <th className="px-4 py-2.5 text-[10px] font-bold text-gray-600 uppercase">Income Source</th>
                                        <th className="px-4 py-2.5 text-[10px] font-bold text-gray-600 uppercase">Buyer</th>
                                ...
                                <tbody>
                                    {incomeList.map((item, idx) => (
                                        <tr key={idx} className="border-b border-gray-50 hover:bg-blue-50 transition-colors group">
                                            <td className="px-4 py-3">
                                                <div className="text-[11px] font-bold text-[#0078d4] font-mono mb-0.5">{item.accCode}</div>
                                                <div className="text-[12px] font-medium text-gray-700 uppercase leading-tight">{item.incomeAccount}</div>
                                            </td>
                                            <td className="px-4 py-3 text-[12px] font-medium text-gray-600 uppercase">{item.buyer}</td>
                                            <td className="px-4 py-3 text-[11px] font-bold text-gray-500 text-center">{item.creditDate}</td>
                                            <td className="px-4 py-3 text-[13px] font-black text-slate-700 text-right">
                        ...
                        <div className="p-2">
                            <div className="max-h-[400px] overflow-y-auto custom-scrollbar">
                                {lookups.filter(a => 
                                    ...
                                    <button key={idx} onClick={() => handleAccountSelect(acc)} className="w-full flex items-center justify-between px-4 py-2.5 border-b border-gray-100 hover:bg-blue-50 transition-all text-left group">
                                        <div className="flex flex-col">
                                            <span className="text-[10px] font-bold text-[#0078d4] font-mono tracking-tighter">{acc.code}</span>
