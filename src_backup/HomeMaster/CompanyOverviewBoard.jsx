...
import {
    X, Building2, Mail, Phone, MapPin, Globe, LayoutDashboard, Activity, FileText, Settings,
...
const CompanyOverviewBoard = ({ company, onClose, onTransactionDeleted }) => {
    ...
    const renderOverview = () => (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                {[
                    { title: 'Total Revenue', value: `Rs ${totalRevenue.toLocaleString(undefined, { minimumFractionDigits: 2 })}`, icon: DollarSign, bgCol: 'bg-emerald-100', textCol: 'text-emerald-600', sub: '+12% this month' },
                    { title: 'Total Transactions', value: txCount, icon: CreditCard, bgCol: 'bg-blue-100', textCol: 'text-blue-600', sub: 'Active processing' },
                    { title: 'Avg. Transaction', value: `Rs ${avgValue.toLocaleString(undefined, { minimumFractionDigits: 2 })}`, icon: TrendingUp, bgCol: 'bg-orange-100', textCol: 'text-orange-600', sub: 'Consistent rate' },
                    { title: 'Active Users', value: company.activeUsers || 0, icon: Users, bgCol: 'bg-purple-100', textCol: 'text-purple-600', sub: 'Logged in today' },
                ].map((stat, i) => (
                    <div key={i} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 hover:shadow-md transition-shadow flex flex-col">
                        <div className="flex items-center gap-3 mb-4">
                            <div className={`w-8 h-8 rounded-full ${stat.bgCol} flex items-center justify-center`}>
                                <stat.icon className={`w-4 h-4 ${stat.textCol}`} />
                            </div>
                        </div>
                        <div className="flex flex-col">
                            <h3 className="text-[24px] font-bold text-gray-800 leading-tight">{stat.value}</h3>
                            <p className="text-[13px] font-medium text-gray-500 mb-4">{stat.title}</p>
                            <div className="flex items-center gap-1.5 text-[12px] font-semibold text-emerald-500 mt-auto">
            ...
            <div className="bg-white border border-gray-100 shadow-sm p-6 mt-6 rounded-2xl">
                <h3 className="text-[11px] font-black tracking-widest uppercase text-gray-500 mb-6 flex items-center gap-2">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {Object.entries(company).map(([key, value]) => {
                        ...
                        return (
                            <div key={key} className="bg-gray-50 p-4 border border-gray-100 shadow-sm flex flex-col justify-center rounded-xl">
                    ...
                    <div className="flex items-center gap-2">
                        {['Week', 'Month', 'Year'].map(t => (
                            <button key={t}
                                ...
                                className={`px-4 py-1.5 text-[10px] uppercase tracking-wider font-black transition-all ${t === timeFilter ? 'bg-blue-600 text-white shadow-sm border border-blue-600 rounded-lg' : 'bg-white text-gray-700 border border-gray-300 hover:bg-gray-50 rounded-lg'
                                    }`}>
                                {t}
                            </button>
                        ))}
                            ...
                            {filteredTransactions.map(tx => (
                                <tr key={tx.docNo} className="hover:bg-gray-50 transition-colors group">
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <span className={`px-3 py-1 text-[9px] font-black uppercase tracking-widest rounded-[3px] border ${tx.payType === 'Cash' ? 'bg-emerald-50 text-emerald-700 border-emerald-300' :
    ...
    return (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/50 p-4">
            <div className="bg-white border border-gray-100 shadow-2xl w-[95vw] max-w-[1500px] h-[90vh] max-h-[900px] flex flex-col overflow-hidden rounded-2xl">
                {/* Modal Header */}
                <div className="p-5 border-b border-gray-200 bg-white flex items-center justify-between shrink-0">
                    <div className="flex items-center gap-3">
                        <div className="w-10 h-10 bg-blue-100 text-blue-600 flex items-center justify-center rounded-xl shadow-sm">
                    ...
                    {tabs.map(tab => {
                        ...
                        return (
                            <button
                                ...
                                className={`flex items-center gap-2 px-5 py-2.5 rounded-lg transition-all text-xs font-bold uppercase tracking-wider whitespace-nowrap ${active ? 'bg-blue-600 text-white shadow-sm border border-blue-600' : 'bg-white text-gray-700 border border-gray-300 hover:bg-gray-50'
                                    }`}
                            >
                                ...
                                <div className="grid grid-cols-2 gap-x-6 gap-y-4">
                                    ...
                                    <div>
                                        ...
                                        <div className="px-4 py-2 bg-gray-50/50 rounded-[3px] border border-gray-200 shadow-sm flex items-center">
                                            <span className={`inline-block px-3 py-0.5 text-[10px] font-black uppercase tracking-widest rounded-[3px] border ${selectedTx.payType === 'Cash' ? 'bg-emerald-50 text-emerald-700 border-emerald-300' :
