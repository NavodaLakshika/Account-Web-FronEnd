...
const SupplierMasterBoard = ({ isOpen, onClose }) => {
    ...
    const handleSave = async () => {
        ...
        if (!formData.Vend_Typ) newErrors.Vend_Typ = 'Vendor Type is required';
    ...
    return (
        <>
                    ...
                    <div className="bg-white p-4 border border-slate-200 rounded-[3px] space-y-4">
                        <div className="grid grid-cols-12 gap-x-6 gap-y-3.5">
                            ...
                            <div className="col-span-8">
                                <label className="block text-[13px] font-medium text-gray-700 mb-1.5">Supplier Name <span className="text-red-500">*</span></label>
                                <input type="text" name="Supplier_Name" value={formData.Supplier_Name} onChange={handleInput} className={`w-full h-10 border ${errors.Supplier_Name ? 'border-red-500' : 'border-gray-300'} rounded-[3px] px-3 text-[14px] bg-white outline-none focus:border-[#0285fd] focus:ring-1 focus:ring-[#0285fd] text-gray-700`} />
                                {errors.Supplier_Name && <div className="text-red-500 text-[11px] mt-1">{errors.Supplier_Name}</div>}
                            </div>
                            ...
                            <div className="col-span-4">
                                <label className="block text-[13px] font-medium text-gray-700 mb-1.5">Vendor Type <span className="text-red-500">*</span></label>
                                <div className="flex gap-2 relative group">
                                    <select
                                        value={formData.Vend_Typ}
                                        onChange={(e) => {
                                            setFormData(prev => ({ ...prev, Vend_Typ: e.target.value }));
                                            if (errors.Vend_Typ) setErrors(prev => ({ ...prev, Vend_Typ: null }));
                                        }}
                                        className={`w-full h-10 border ${errors.Vend_Typ ? 'border-red-500' : 'border-gray-300'} rounded-[3px] px-3 text-[14px] bg-white outline-none focus:border-[#0285fd] focus:ring-1 focus:ring-[#0285fd] text-gray-700 uppercase cursor-pointer appearance-none`}
                                        style={{ backgroundImage: `url("data:image/svg+xml;charset=UTF-8,%3csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='none' stroke='%2364748b' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'%3e%3cpolyline points='6 9 12 15 18 9'%3e%3c/polyline%3e%3c/svg%3e")`, backgroundRepeat: 'no-repeat', backgroundPosition: 'right 0.5rem center', backgroundSize: '1em' }}
                                    >
                                ...
                                {errors.Vend_Typ && <div className="text-red-500 text-[11px] mt-1">{errors.Vend_Typ}</div>}
                            </div>
            ...
            <ConfirmModal
                ...
                message={`Are you sure you want to delete this supplier (${formData.Supplier_Name || formData.Code})? This action cannot be undone.`}
                loading={loading}
                confirmText="Delete"
                variant="danger"
            />
