...
import { Palette, Layout, Moon, Eye, Shield, Check, Maximize, ZapOff, Type, CircleDashed, Square, Sun, Coffee } from 'lucide-react';
...
const FormSettingsModal = ({ isOpen, onClose, settings, onSettingsChange }) => {
    ...
    return (
        <div ref={dropdownRef} className="absolute right-0 top-full mt-3 z-[600] w-[280px] bg-white rounded-[4px] shadow-[0_10px_40px_-10px_rgba(0,0,0,0.15)] border border-gray-200 animate-in fade-in slide-in-from-top-2 duration-150 font-['Tahoma'] cursor-default max-h-[70vh] overflow-y-auto no-scrollbar">
            ...
            <div className="px-4 py-3 border-b border-gray-100 bg-slate-50/50 rounded-t-[4px] flex items-center gap-2">
                ...
                <h4 className="text-[11px] font-black text-gray-600 uppercase tracking-widest">Form Display</h4>
            </div>

            {/* Content */}
            <div className="py-1">
                <SettingRow icon={Moon} title="Dark Mode" stateKey="darkMode" settings={settings} toggleSetting={toggleSetting} />
